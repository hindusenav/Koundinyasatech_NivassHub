...
// /// Screen 4 — collects the registered email address and requests an OTP.
...
//       identifier: email,
//       channel: ForgotPasswordChannel.email,
//     );
//     if (!mounted) return;
...
class _ForgotPasswordEmailScreenState extends State<ForgotPasswordEmailScreen>
  ...
  Future<void> _handleSendOtp() async {
    ...
    try {
      // Call the API directly for email
      final response = await _service.generateOTP(
        umail: email,
        action: 'GENERATE', // No cont_code needed for email
      );
      ...
      if (response.success && response.otpKey != null) {
        ...
        forgotPassword.setOtpData(
          ...
          otpType: response.otpType ?? 'EMAIL',
          channel: ForgotPasswordChannel.email,
        );
