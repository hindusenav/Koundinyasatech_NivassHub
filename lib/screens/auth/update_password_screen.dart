...
class _UpdatePasswordScreenState extends State<UpdatePasswordScreen>
  ...
  bool get _canSubmit {
    final password = _passwordController.text;
    final confirm = _confirmPasswordController.text;
    final allRequirementsMet = evaluatePasswordRequirements(
      password,
    ).values.every((v) => v);
    return allRequirementsMet && confirm.isNotEmpty && confirm == password;
  }
  ...
  Future<void> _handleSubmit() async {
    ...
    // Get FP_Token from provider (or use widget's fpToken)
    final fpToken = forgotPassword.fpToken ?? widget.fpToken;
    ...
    try {
      ...
      if (response.httpStatus == 200) {
        ...
        // Navigate to login and clear the entire auth stack
        NavigationService.pushNamedAndRemoveUntil(AppRoutes.login);
  ...
  void _handleError(int statusCode, String message) {
    if (statusCode == 401) {
      ...
      CustomSnackbar.error(
        context,
        'Password update conflict. Please try again.',
      );
  ...
  Widget build(BuildContext context) {
    ...
    final contentWidth = context.isDesktop
        ...
        : (context.isTablet
              ...
              : double.infinity);
    final isUpdating = context.watch<ForgotPasswordProvider>().isSendingOtp;
                            ...
                            children: [
                              ...
                              Opacity(
                                ...
                                child: AuthGradientButton(
                                  label: 'Update Password',
                                  isLoading: isUpdating,
                                  onPressed: (_canSubmit && !isUpdating)
                                      ? _handleSubmit
                                      : null,
                                ),
                              ),
