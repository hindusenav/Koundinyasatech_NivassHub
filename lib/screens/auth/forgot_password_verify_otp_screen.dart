...
// lib/screens/auth/forgot_password_verify_otp_screen.dart
...
class _ForgotPasswordVerifyOtpScreenState
  ...
  Future<void> _handleVerify() async {
    ...
    // Get OTP data from provider
    final otpKey = forgotPassword.otpKey;
    final identifier = forgotPassword.identifier;
    final otpType = forgotPassword.otpType;
    ...
    // Set verifying state
    forgotPassword.setVerifyingOtp(true);
    ...
    try {
      ...
      if (response.success &&
          response.fpToken != null &&
          response.fpToken!.isNotEmpty) {
        // Store FP_Token in provider
        forgotPassword.setFpToken(response.fpToken!);

        // Navigate to update password screen
        Navigator.pushReplacementNamed(
          context,
          AppRoutes.updatePassword,
          arguments: UpdatePasswordScreenArgs(fpToken: response.fpToken!),
        );
      ...
      if (mounted) {
        forgotPassword.setVerifyingOtp(false);
      }
    }
  }
  ...
  Future<void> _handleResend() async {
    ...
    final identifier = forgotPassword.identifier ?? widget.identifier;

    // Check if resend is allowed
    if (!forgotPassword.canResend()) {
      final remaining = forgotPassword.getRemainingCooldownSeconds();
      CustomSnackbar.error(
        context,
        'Please wait $remaining seconds before requesting another resend.',
      );
      return;
    }

    // Set resending state
    forgotPassword.setResendingOtp(true);
    forgotPassword.clearError();
    ...
    try {
      ...
      final response = await _service.generateOTP(
        umail: identifier,
        contCode: widget.channel == ForgotPasswordChannel.mobile ? '+91' : null,
        action: 'RESEND',
      );
      ...
      if (response.success && response.otpKey != null) {
        // Update provider with new OTP data
        ...
        // Increment resend count and restart cooldown
        forgotPassword.incrementResendCount();
        _startCountdown();
      ...
      if (mounted) {
        forgotPassword.setResendingOtp(false);
      }
    }
  }
  ...
  void _handleError(int statusCode, String message) {
    if (statusCode == 429) {
      ...
      CustomSnackbar.error(
        context,
        'Invalid OTP or session. Please try again.',
      );
                          ...
                          children: [
                            ...
                            Opacity(
                              ...
                              child: _secondsRemaining > 0
                                  ...
                                  : GestureDetector(
                                      onTap: forgotPassword.isResendingOtp
                                          ? null
                                          : _handleResend,
                                      child: Text(
                                        forgotPassword.isResendingOtp
                                            ? 'Resending...'
                                            : 'Resend OTP',
